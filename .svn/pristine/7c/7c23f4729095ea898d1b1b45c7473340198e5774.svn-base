; (function ($) {
    $.extend({
        ...
        "searchExt":
                    function (_option) {//文本框对象，按钮对象
                        _option = $.extend({
                            ...
                            callback: function (data) { if (data.result) { alert("操作成功") } else { alert(data.info) } },//AJAX成功后回调函数
                            errorcallback: function (XMLHttpRequest, textStatus, errorThrown) { alert("eror"); },//AJAX失败后回调函数
        ...
        saveExt: function (_option) {
            var option = $.extend(true, { url: undefined, data: null, callback: function () { }, onBefor: function () { return true; } }, _option);
            if (option.onBefor()) {
                if (option.url == undefined) { top.$.messager.alert("系统提示", "不明确请求地址！", "info"); return false; }
        ...
        savecustomExt: function (_option) {
            var option = $.extend(
                ...
                _option);
            if (option.url == undefined) { top.$.messager.alert("系统提示", "不明确请求地址！", "info"); return false; } else {
                $.ajax(option);
                return true;
            }
        },
        ...
        dialogExt: function (_option) {
            ...
            _option = $.extend({
                isClose: false,
                callback: function () {
                    if (_option.isClose) { win.dialog("close"); $(win).remove(); }
                    if (_option.onBack) _option.onBack();
                }
            }, _option);
            var option = $.extend({
                title: '标题',
                ...
                modal: true,
                onLoad: function () { },
                onClose: function () { $(win).remove(); },
                buttons: [{
                    text: "保存", iconCls: "icon-save", handler: function () {
                        if (win.form("validate")) {
                            var saveurl = '';
                            if (option.saveurl) {
                                saveurl = option.saveurl;
                            } else {
                    ...
                    handler: function () { win.dialog("close"); $(win).remove(); }
                }]
            }, _option);
            win.dialog(option);
        },
        ...
        loadingExt: function (_option) {
            ...
            if (_option.isFullScreen) { temp.css("z-index", "9000") }
            msg.css("z-index", "90001")
            $(_option.appendObj).prepend(msg).prepend(temp);
        },
        loadedExt: function () {
            if ($("#WindowLoading").length > 0) { $("#WindowLoading").remove(); }
            if ($("#WindowLodingMsg").length > 0) { $("#WindowLodingMsg").remove(); }
        }
    });
    ///对象扩展 
    $.fn.extend({
        ...
        "formSerializeEscapeExt":
                                    function () {
                                        ...
                                        $.each(a, function (i) {
                                            if ($("input[name=" + a[i].name + "]").val() == $("input[name=" + a[i].name + "]").attr("title"));
                                            else o[a[i].name] = escape(a[i].value) || '';
                                        });
                                        return o;
                                    },
        ...
        extGrid: function (option) {
            ...
            var _option = $.extend({
                ...
                columns: [],
                onLoadError: function () { alert("加载数据出错……"); }
            }, option);
        ...
        gridEdit: function (_option) {
            ...
            if (_option.idField) {
                if (_option.href) { linkSign = _option.href.indexOf("?") == -1 ? "?" : "&"; }
                var rows = _this.datagrid("getSelections");
                if (rows.length == 0 && !_option.isadd) { top.$.messager.alert("系统提示", "请选择一条记录进行操作!", "warning"); return; }
                if (rows.length <= 1) {
                    if (rows.length == 1) {
                        if (typeof (_option.idField) == "string") {
                            ...
                            var param = {};
                            $.each(_option.idField, function (i, item) { param[item] = rows[item]; });
                            if (_option.href)
                    ...
                    { top.$.messager.alert("系统提示", "一次只能编辑一条记录!", "warning"); return; }
                }
            }
            _option = $.extend({
                isClose: false,
                callback: function () {
                    _this.datagrid("clearSelections"); _this.datagrid("reload");
                    if (_option.isClose) { win.dialog("close"); $(win).remove(); }
                    if (_option.onBack) _option.onBack();
                }
            }, _option);
        ...
        gridTreeEdit: function (_option) {
            ...
            _option = $.extend({
                isClose: false,
                callback: function () {
                    _this.treegrid("clearSelections"); _this.treegrid("reload");
                    if (option.isClose) { win.dialog("close"); $(win).remove(); }
                    if (option.onBack) option.onBack();
                }
            }, _option);
            var win = $("<div/>");
            var option = $.extend({
        ...
        gridRemove: function (_option) {
            var param = {}; var _this = this; var sltRow;
            if (_option.idField) {
                if (typeof (_option.idField) == "string") {
                    sltRow = _this.getSelected(_option.idField);
                    if (sltRow == null) { top.$.messager.alert("系统提示", "请选择一条记录进行操作!", "warning"); return; }
                    $.each(sltRow, function (i, value) { param["param[" + i + "]"] = '\'' + value + '\''; });
                }
                else {
        ...
        treeEdit: function (_option) {
            ...
            if (true) {
                ...
                if (rows == null && !_option.isadd) { top.$.messager.alert("系统提示", "请选择一条记录进行操作!", "warning"); return; }
                if (rows != null) {
        ...
        treeRemove: function (_option) {
            ...
            _option = $.extend({
                callback: function () {
                    this.tree('reload');
                    if (_option.onBack) _option.onBack(nodes);
                }
            }, _option);
            if (nodes.length == 0) { $.messager.alert("系统提示", "请选择一条记录进行操作!", "warning"); return; }
            if (nodes.length > 0) {
                var _this = this;
...
var ManageOutDiv = function (_options) {
    var option = $.extend({
        ...
        height: 300,
        href: '',
        onCollapse: function () { alert('折叠'); },
        onBeforeCollapse: function () { alert('折叠之前') },
        onClose: function () { $(top.$.find('body')).find('#CommOutDiv').remove(); },
        onMaximize: function () { alert('点了最大化'); },
                        ...
                        if (outidiv.form("validate")) {
                            ...
                            var saveoption = $.extend({
                                ...
                                callback: function () {
                                    ...
                                    if (_options.onBack) _option.onBack();
                                }
                            }, _options);
                            var saveoption = $.extend(true, { url: undefined, data: null, callback: function () { }, onBefor: function () { return true; } }, saveoption);
                            if (saveoption.onBefor()) {
                                if (saveoption.url == undefined) { top.$.messager.alert("系统提示", "不明确请求地址！", "info"); return false; }
