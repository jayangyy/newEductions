...
public class FileUtil {  
 //   public static Log logger = Logger.getLogger(FileUtil.class);  
     ...
     * @param strPath 文件地址
     */  
    public static void removeFiles(String strPath,boolean isDelDir) {  
        ...
        for (int i = 0; i < fileList.length; i++) {  
            ...
            if (tempFile.isDirectory()) {  
                removeFiles(strPath + "/" + fileList[i],false);// 下删除文件夹里面的文件  
                removeFolder(strPath + "/" + fileList[i]);// 删除文件夹  
            }  
        }  
        if(isDelDir){ file.delete();}
    }  
    /** 
     * 写入文件 
     *  
  ...
  public static void insertFile(String savePath,InputStream inputstream) throws IOException{
     ...
     * @param strPath 文件保存绝对路径地址
     */ 
  public static void insertFileBytes(String savePath, byte[] buffer) throws IOException{
	  int bytesum = 0;  
      int byteread = 0;  
	  @SuppressWarnings("resource")
	  FileOutputStream fs = new FileOutputStream(savePath);  
